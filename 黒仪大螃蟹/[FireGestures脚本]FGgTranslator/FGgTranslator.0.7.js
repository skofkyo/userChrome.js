		...
		window.FGgTranslator = {
			...
			get getPlayList (){
				var str = this.camelCaseText,
					...
					strArr2 = [],
					index = 0;
				for(var i=0; i<this.camelCaseText.length; i++){
				...
				if(!this.player){
					...
					this.player.onended = function(){
						...
						if(this.pIndex == PL.length){
							...
							PS.get(this.pIndex);
							//console.log(decodeURIComponent(that.getPlayList[this.pIndex]).replace(that.link + "translate_tts?&q=", "").replace("&tl=" + that.checkLanguge + "&prev=input", ""));
