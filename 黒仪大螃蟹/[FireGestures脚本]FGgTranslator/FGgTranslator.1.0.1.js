...
// @Note           [141122]添加bing, baidu翻译服务。[141123]修正baidu翻译一处错误。
// ==/FireGesturesScript==
		...
		window.FGgTranslator = {
			...
			baiduService: function(res, details){
				...
				sentRequest.call(this, details, function(res){
					...
					if(dictResult){
						...
						if(items[3])
							baike = '<div class="_FgGTr-bd-baike"><a href="'+ items[3].link +'" target="_blank">' + items[3].content.replace(/\&amp;/g, '&') + '</a></div>';


						//标签内容
						for(var tc of [net, cizu, tongfanyici, baike])
							if(tc) tContent += '<li>' + tc + '</li>';
			...
			setResultLink: function(){
				...
				if(this.service == 'google'){
					link.href = (this.google.indexOf("translate") < 0 
							...
							: this.google) + "?text="+ 
						encodeURIComponent(obj.text || this.camelCaseText) +"&langpair="+
					...
					link.href = 'http://fanyi.baidu.com/#'
						+ obj.from + '/' + obj.to + '/' + encodeURIComponent(obj.text);
				}
			},
			...
			get getPlayList (){
				...
				if(this.service == 'google'){
					...
					return ['http://api.microsofttranslator.com/v2/http.svc/speak?appId='+ this.bingAppId +'&language='+this.checkLanguge +'&format=audio/mp3&options=MinSize&text='+str];
				}else if(this.service == 'baidu'){
			...
			setStyle: function(element){
				var style = document.createElement('style'),
					cssText = (function(){/*
						...
						._FgGTr-bd-tongfanyici>li>h6::before,
						._FgGTr-bd-part-name>li>h6::before,
