...
*/
	var StylishCustom = {
		...
		oldPreview: null,
		init: function () {
			eval("save=" + save.toString().replace("return true;", "StylishCustom.oldPreview = codeElementWrapper.value;document.getElementById('unperview').setAttribute('disabled',true);$&"));
			eval("preview=" + preview.toString().replace("setTimeout", "document.getElementById('unperview').setAttribute('disabled',false);$&"));
			...
			document.getElementById("editor").addEventListener("keydown", function (e) {
				...
				if (e.ctrlKey && e.keyCode == 68) {//"d"
					...
					var v = codeElementWrapper.value;
						...
						sl = v.substr(0, s).split("\n").length - 1,
						vs = v.split("\n");
					vs.splice(sl,0,vs[sl]);
					codeElementWrapper.value = vs.join("\n");
					codeElementWrapper.focus();
					codeElementWrapper.setSelectionRange(s, s);
				}
			}, false)
		},
