...
location == 'chrome://browser/content/browser.xul' && (function () {
	var querySearch = {
		description: [
			'shift	+ 鼠标左键：使用“关键词 site:当前域名”搜索',
			'鼠标中键：弹出框搜索。',
			'shift + 鼠标中键：弹出框使用“关键词 site:当前域名”搜索',
			'鼠标右键： 搜索粘贴板',
		...
		getSearchText: function(isClipboard) {
			...
			if(isClipboard) {
				...
				selection = (gBrowser.contentWindow || gBrowser.contentWindowAsCPOW).getSelection().toString();
				if(!selection){
					let isOnTextInput = function (elem) {
						return elem instanceof HTMLTextAreaElement || 
								(elem instanceof HTMLInputElement && elem.mozIsTextField(true));
					};
					if(isOnTextInput(gContextMenu.target)){
						selection = gContextMenu.target.QueryInterface(Ci.nsIDOMNSEditableElement)
