	...
	var reloadAndstopButton = {
		...
		init: function(){
			...
			var cssStr = '@-moz-document url("chrome://browser/content/browser.xul"){'
						+'#reloadAndstopButton .toolbarbutton-icon {'
						+'list-style-image: url("chrome://browser/skin/reload-stop-go.png");'
						+ '-moz-image-region: rect(0, 14px, 14px, 0);'
						+ '}'
						+ '#reloadAndstopButton.reloadAndstopButton-stop .toolbarbutton-icon {'
		...
		updateBtnUI: function(add){
			if(CustomizableUI.getWidget("reloadAndstopButton").areaType){
				this.anchorBtn = this.anchorBtn 
								|| CustomizableUI.getWidget("reloadAndstopButton").instances[0].anchor;
