...
var tiebaNotification = {
			...
			while(wm.hasMoreElements()){
				...
				if(this._button){
					...
					if(typeof PrivateBrowsingUtils.isWindowPrivate == 'function'){
						if(PrivateBrowsingUtils.isWindowPrivate(cWin) 
							!= PrivateBrowsingUtils.isWindowPrivate(win)) continue;
					}else{
						if(PrivateBrowsingUtils.isBrowserPrivate(cWin.gBrowser) 
							!= PrivateBrowsingUtils.isBrowserPrivate(win.gBrowser)) continue;
					}
				}
				if(!!value)
	...
	popupPanel: function(event, message){
		...
		for(var i=0; i<message.length; i++){
			...
			li.onclick = (function(link){
				return function(){
					var c = li.textContent.match(/^\d+/);
					gBrowser.loadOneTab(link, null, null, null, false, false);
					if(!c) return;
					count -= parseInt(c[0]);
					setBadge.call(that, count);
					li.remove();
					_message.count = count;
					count == 0 && panel.hidePopup();
				};
			})(links[i], li, count);
			li.innerHTML = message[i].replace(/^\d+/, '<b>$&</b> ');
			ul.appendChild(li);
		}
	...
	showNotification: function(message, listener){
		clearTimeout(this._showNotificationTimeout);
		this._showNotificationTimeout = setTimeout(function(){
	...
	getTiebaMessage: function(resolve, reject){
		...
		var {showNotification, info, setBadge, setInterval, popupPanel} = this,
			isPromise = (typeof reject == 'function'),
		showNotification = showNotification.bind(this);
		setBadge = setBadge.bind(this);
		popupPanel = popupPanel.bind(this);
		setInterval(false);
		this.ajax({
			url: 'http://message.tieba.baidu.com/i/msg/get_data?t=' + new Date().getTime(),
			method: 'GET',
			timeout: 5000,
			ontimeout: function () showNotification(info.neterr),
			onreadystatechange :function (req) {
				req = req.target;
				if(req.readyState != 4) return;
				if(req.status != 200) return showNotification(info.neterr);
				var n = req.responseText.match(/\d+/g);
				if(!n || n.length != 23) {
					showNotification(info.expired);
				...
				var message = [],
					links = [],
					count = 0,
					tblink = 'http://tieba.baidu.com/i/sys/jump?u='+ (new Date().getTime()) + '&type=';
				for(var i=0; i<n.length; i++){
					if(n[i] != 0 && (i in info.type)){
						count += n[i];
						message.push(n[i] + '个' + info.type[i][1]);
						links.push((i == 9 ? 'http://tieba.baidu.com/pmc/': tblink) + info.type[i][0]);
					}
				}
				if(message.length == 0) return isPromise && reject();

				setBadge(count);
				info._message = [message, links, count];
				
				showNotification('你共有'+ count + '条信息未读。\n\t' + message.join(', \n\t'), {
					observe: function(subject, topic, data) {
						if(topic == 'alertclickcallback'){
							for(var i of links)
								gBrowser.loadOneTab(i, null, null, null, false, false);
							setBadge(0);
						}
					}
				});
	...
	ajax: function(obj){
		...
		req.send(null);
		req.onreadystatechange = obj.onreadystatechange;
		return req;
	},
