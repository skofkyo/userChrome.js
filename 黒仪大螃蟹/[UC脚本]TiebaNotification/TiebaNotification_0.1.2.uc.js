...
// @version        0.1.2
// @note           [20141103]添加关闭通知面板的选项，改进tooltip显示，和修正一些错误。
// ==/UserScript==
...
var tiebaNotification = {
		...
		for(var i=0; i<message.length; i++){
			...
			li.onclick = (function(link){
				return function(){
					...
					if(!c) return; count -= parseInt(c[0]);
					let index = message.indexOf(li.text);
					!!~index && message.splice(index, 1);
	...
	getTiebaMessage: function(resolve, reject){
		...
		var {popupNotification, info, setBadge, setInterval, popupPanel} = this,
			isPromise = (typeof reject == 'function'),
		popupNotification = popupNotification.bind(this);
		...
		this._tiebaMessageRequest = this.ajax({
			...
			onload :function (req) {
				req = req.target;
				if(req.status != 200) return popupNotification(info.neterr);
				var n = req.responseText.match(/\d+/g);
				if(!n || n.length != 23) {
					popupNotification(info.expired);
				...
				popupNotification('你共有'+ count + '条信息未读。\n\t' + message.join(', \n\t'), {
