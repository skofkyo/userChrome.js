	...
	var ctrlAndRightClickPicture ={
		...
		getPicture: function(event){
			...
			if(event.altKey){
				...
				while(targetNode != null){
					...
					if(targetNodeStyle.backgroundImage != "none"){
						var targetNodeBg = targetNodeStyle.backgroundImage.match(/http[^"')]+/);
						if(targetNodeBg){
			...
			for(var img of imgs){
				...
				if((img.naturalWidth > 4 && img.naturalHeight > 4 || targetStyle.backgroundImage != "none") 
					...
					var imgurl;
					if(targetStyle.backgroundImage != "none"){
						var imgurl_match = targetStyle.backgroundImage.match(/http[^"')]+/);
