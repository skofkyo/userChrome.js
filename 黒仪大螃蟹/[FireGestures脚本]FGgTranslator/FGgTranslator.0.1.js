...
// @compatibility  FF26+
// @version        2014.01.22.0.01
// @Note           需要安装FireGestures扩展。
// ==/FireGesturesScript==
	if(!window.FGgTranslator){
		window.FGgTranslator = {
			link: "https://translate.google.com.hk/",//可以换一个比较通畅的google翻译服务器地址
			selectText: null,
			boxElements:null,
			player: null,
			to: "zh-CN",
			...
			init: function(event){
				...
				this.boxElements.box.style.top = event.clientY + window.pageYOffset-20 +"px";
				this.boxElements.box.style.left = event.clientX + window.pageXOffset-50 +"px";

				this.setTranslateText();
			},

			setTranslateText: function(word){
				word = word || (this.camelCase ? this.selectText
				...
				function ftt(text, title){
					return '<span'+(title ? (' title="原文:\n\t'+ sF(title) +'"') : '')
					...
					onload : function(res) {
						...
						if (res.status == 200) {
							...
							if(!res.responseText || !text[0]){
								return this.statusAlert("未知错误", true);
							}
							//原文所属语言
							...
							//清空原来的
							if(this.boxElements.detail.children.length){
							...
							this.statusAlert("错误：访问google翻译服务器出错。", true);
						}else if(res.status == 414){
							this.statusAlert("错误：要翻译的文本过长。", true);
						}
					}.bind(this)
				});
			},
				...
				this.boxElements = {
					...
					setResultLink: function(obj) {
						this.resultBox.parentNode.href = 
								that.link + "?text="+ 
								encodeURIComponent(obj.selectText || that.selectText) +"&langpair="+
								(obj.from || that.from) +"|"+
								(obj.to || that.to);
					},
				};
			...
			playSound: function(){
				var url = this.link + "translate_tts?&q=" 
								+ encodeURIComponent(this.selectText)
								+ "&tl=" + this.checkLanguge + "&prev=input";
				if(!this.player || this.player.getAttribute("src") != url ){
					this.ajax({
						method: 'GET',
						url: url,
						onload: function(res) {
							...
							if (res.status == 200) {
								...
								this.statusAlert("错误：无此语音，或文本过长。");
							}
						}.bind(this)
					});
				}else{
			...
			statusAlert: function(text, repeat){
				...
				this.boxElements.alertBox.hideTimer = setTimeout(function(){
					try{
						repeat || this.setClassName(this.boxElements.alertBox, 
			...
			getSelection: function(event){
				...
				if(event && Array.prototype.slice
					.call(document.querySelectorAll("TEXTAREA, input"))
						.some(function(item){
