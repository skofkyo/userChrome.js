	...
	if(!window.FGgTranslator){
		window.FGgTranslator = {
			...
			getBingAppId: function(callback){
				this.sentRequest({
					url: 'http://www.bing.com/translator/dynamic/210010/js/LandingPage.js?gt=',
					rqType: 'bingGetAppId'
				}, function(res){
					...
					if(tt_r){
						...
						for(var p of tRD){
							for(var ci of p.result){
								...
								rt += ftt((this.checkLanguge != 'en' 
									&& /[a-z\s\d]\b$/i.test(ci[1])) ? ci[1] + ' ' : ci[1], //译文为英文时要加空格
					...
					if(td_r && ((typeof td_r == 'object') && !Array.isArray(td_r) || dictResult)){
						...
						if(td_r.synthesize_means && td_r.synthesize_means.symbols 
							...
							for(var i of td_r.synthesize_means.symbols){
								synthesize += '<li>';
								if(i.xg != ''){
									synthesize += '<h5>' + td_r.synthesize_means.word_name 
										+ ' <span>[' + i.word_symbol + ']</span></h5>';
								}
							...
							tongfanyici = (function(tfyc){
								...
								for(var tf in tfyc){
									if(tfyc[tf]){
										...
										if(str != '') ul.push('<h5>'+ ('同反'.charAt(tf)) 
						...
						//获取中中、英英析义
						//details.url = details.url.replace(/v2transapi$/, 'extendtrans');
						details.headers['User-Agent'] = 'Android';//必须安卓UA
						details.url = 'http://app.fanyi.baidu.com/agent.php?plat=android&version=25&imei=&product=transapp&cuid=|&req=newtranshtml&type=json&trans_mode=3&channel=baiduas&' + details.postData;
						details.method = 'GET';
						details.rqType = 'baiduExtendtrans';
						sentRequest(details, function(res){
							var text = res.responseText;
							if(text == '') return setTab();
							try{
								text = JSON.parse(text);
							}catch(ex){
								return setTab();
							...
							if(text.zdict){
								tName['中中析义'] = (function(zdict){
									var arr = [];
									for(var zd in zdict){
										var xiyi = '';
										if(!zdict[zd] || !zdict[zd].means) break;
										xiyi += '<ul class="_FgGTr-bd-zdictMs">';
										for(var means of zdict[zd].means){
											xiyi += '<li>';
											if(zd == 0 && means.pinyin)
												xiyi += '<h5>' + text.zdict.word + ' <span>[' + means.pinyin + ']</span></h5>';
									...
									arr = arr.join('</li><li><h5>引证解释</h5>');
									return arr ? '<ul class="_FgGTr-bd-zdict"><li>' + arr + '</li></ul>': '';
								}).call(this, [text.zdict.simple, text.zdict.detail]);
							}
							if(text.edict && text.edict.item && text.edict.item.length){
								var enxiyi = '';
								for(var item of text.edict.item){
								...
								if(enxiyi != '') tName['英英析义'] = '<ul class="_FgGTr-bd-edict">' + enxiyi + '</ul>';
							}
							setTab();
						}.bind(this));
					}
			...
			setTranslateText: function(word){
				...
				//消除闪烁
				this.setClassName(this.boxElements.box, '_FgGTr-AntiBlink', true);
				...
				if(this.service == 'google'){
					...
					details.url = 'http://api.microsofttranslator.com/v2/ajax.svc/TranslateArray2?appId=%22'+ this.bingAppId +'%22&texts='+ encodeURIComponent(JSON.stringify(word.split('\n'))) +'&from=%22'+ (this.from == 'auto' ? '' : this.from) +'%22&to=%22' + this.to + '%22&options=%7B%7D&oncomplete=onComplete_4&onerror=onError_4&_=';
			...
			sentRequest: function(details, callback){
				...
				resultBox.ajaxRequest = this.ajax({
					...
					ontimeout: function(e){
						if(rqType == 'baiduExtendtrans') return callback && callback(e);
						ld && this.loadingAnimation();
			...
			setScrollbar: function(element){
				...
				bdTab && this.setClassName(bdTab.parentNode, '_FgGTr-AntiBlink', true);
				setTimeout(function(){
					...
					if(contentHeight < 250) {
						bdTab && this.setClassName(bdTab.parentNode, '_FgGTr-AntiBlink', false);
						this.setClassName(this.boxElements.box, '_FgGTr-AntiBlink', false);
						return;
					}
			...
			setStyle: function(element){
				var style = document.createElement('style'),
					cssText = (function(){/*
						...
						._FgGTr-bd-synthesize>li>h5>span,
						...
						._FgGTr-bd-synthesize>li>h5 + ._FgGTr-bd-synthesizeCys>li>h6,
