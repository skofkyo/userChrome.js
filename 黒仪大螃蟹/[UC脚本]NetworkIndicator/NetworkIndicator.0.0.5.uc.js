...
if (location == 'chrome://browser/content/browser.xul') {
	...
	const GET_LOCAL_AND_PUBLIC_IPS = true; //是否启用获取显示内外网IP。基于WebRTC，
	...
	let networkIndicator = {
		...
		_getLocalAndPublicIPs: GET_LOCAL_AND_PUBLIC_IPS,
			...
			if(obj.location){
				...
				let cE = this.createElement,
					...
					for(let i of info){
						let p = cE('p', {}, li);
						if(i.text == '外网IP: '){
							this.setTooltip(p,  {ip: i.value, location: this.localAndPublicIPs.publicLocation});
						}
						cE('span', {text: i.value + '\n'}, cE('span', {text: i.text}, p).parentNode);
					}
					list.insertBefore(fm, list.firstChild);
					//同时更新第一个（主域名）tooltip
					this.setTooltip(list.querySelector('.ucni-MainHost'), obj);
				};
				info.push({value: obj.location, text: '所在地: '});
				obj.server && info.push({value: obj.server, text: '服务器: '});

				if(this._getLocalAndPublicIPs && (!this.localAndPublicIPs || timeStamp - this._lastRTCPCTimeStamp > 30 * 60 * 1000)){
					this._lastRTCPCTimeStamp = timeStamp;
					(new Promise(this.getLocalAndPublicIPs)).then(reslut => {
						this.localAndPublicIPs = reslut;
						info.push({value: reslut.localIP, text: '内网IP: '});
						info.push({value: reslut.publicIP, text: '外网IP: '});
						updateMainInfo(info);
					}, () => {updateMainInfo(info);}).catch(() => {
						updateMainInfo(info);
					});
				}else{
					if(this.localAndPublicIPs){
						info.push({value: this.localAndPublicIPs.localIP, text: '内网IP: '});
						info.push({value: this.localAndPublicIPs.publicIP, text: '外网IP: '});
					}
					updateMainInfo(info);
				}
			}else{
		...
		getLocalAndPublicIPs(resolve, reject){
			...
			hiddenFrame.get().then(window => {
				...
				let regex = /UDP \d+ ([\da-z\.\:]+).+srflx raddr ([\da-z\.\:]+)/i;
				pc.onicecandidate = ice => {
					if(!ice.candidate) return;
					let ip = ice.candidate.candidate.match(regex);

					if(DEBUG) debug.push(ice.candidate.candidate);

					if(Array.isArray(ip) && ip.length == 3){
						clearTimeout(_RTCtimeout);
						hiddenFrame.destroy();
						hiddenFrame = null;
						return resolve({publicIP: ip[1], localIP: ip[2]});
					}
				};
		...
		handleEvent(event){
			switch(event.type){
				case 'TabSelect':
					...
					break;
				case 'dblclick':
					let info = this.panel._list.querySelectorAll('#ucni-mplocation > p:nth-child(2n)');
					if(Array.some(info, e => e.contains(event.originalTarget))){
						let ipText = info[info.length - 1].childNodes[1].textContent.trim();
						if(/^[a-z\.\:\d]+$/i.test(ipText)){
							this.queryLocation(ipText, result => {
								this.localAndPublicIPs.publicLocation = result.location;
								let text = this.setTooltip(info[info.length - 1], result);
								if(event.altKey)
									this._nsIClipboardHelper.copyString(text);
							});
						}
		...
		createElement(name, attr, parent){
			let ns = '', e = null;
			if(!!~['panel', 'image', 'menupopup', 'menuitem', 'vbox'].indexOf(name)){
